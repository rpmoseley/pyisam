...
vdumptree (struct VBTREE *pstree, int iindent)
{
    ...
    printf ("%-*.*sNODE:%ld  \t%08lX DAD:%04lX LVL:%04X CUR:%04lX",
     iindent, iindent, "          ",
     pstree->tnodenumber,
     (intptr_t)pstree,
     (intptr_t)pstree->psparent & 0xffff,
     pstree->ilevel, (intptr_t)pstree->pskeycurr & 0xffff);
    fflush (stdout);
    if (pstree->iisroot) {
        printf (" RT.");
    }
...
#if ISAMMODE == 1
    printf
    ("%-*.*s KEY :%02X%02llX\t%08lX DAD:%04lX KID:%04lX ROW:%04lX",
     iindent, iindent, "          ",
     ikey,
     pskey->tdupnumber,
     (intptr_t)pskey,
     (intptr_t)pskey->psparent & 0xffff,
     (intptr_t)pskey->pschild & 0xffff, pskey->trownode & 0xffff);
#else	/* ISAMMODE == 1 */
    printf
    ("%-*.*s KEY :%02X%02llX\t%08lX DAD:%04lX KID:%04lX ROW:%04lX",
     iindent, iindent, "          ",
     ...
     (intptr_t)pskey,
     (intptr_t)pskey->psparent & 0xffff,
     (intptr_t)pskey->pschild & 0xffff, (intptr_t)pskey->trownode & 0xffff);
#endif	/* ISAMMODE == 1 */
...
ichktree (int ihandle, int ikeynumber)
{
    ...
    printf("CURR KEY :%02X%02lX\t%08lX DAD:%04lX KID:%04lX ROW:%04lX",
     cbuffer[0],
     pskey->tdupnumber,
     (intptr_t)pskey,
     (intptr_t)pskey->psparent & 0xffff,
