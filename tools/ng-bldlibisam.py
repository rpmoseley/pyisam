...
This version is able to change the destination directory depending on which build
format is being used, the default is to populate a local build directory.

Possible settings:

--bldmode=local
    Populate the directory 'build' instead of '/tmp/pyisam'.
--bldmode=subproject
    Populate the appropriate directories under the provided BUILD directory
'''
...
# Define a configuration object in which to place the various settings.
...
def parse_args(config, args=None):
  'Parse the arguments updating the CONFIG object, using sys.argv[] if no ARGS given'
  parser = argparse.ArgumentParser(
    prog='bldlibisam',
    description='Build the necessary support libraries for the pyisam package'
  )
  parser.add_argument('--local', 
    dest='bldmode',
    help='Populate local build directory',
    action='store_const',
    const='local',
  )
  parser.add_argument('--subproject', 
    dest='bldmode',
    help='Populate a meson based build directory',
    nargs=1,
    type=pathlib.Path,
  )
  ...
  # Check if a build directory was specified if required
  if config.bldmode == 'subproject' and config.builddir is None:
    raise ValueError('Must specify a build directory in subproject mode')

def setup_build(config):
  'Perform the necessary changes to enact the build mode specified'
  match config.bldmode in:
    case 'local':
      topdir = pathlib.Path(__file__).parent.parent
      config.workdir = topdir / 'build/pyisam'
      config.instdir = topdir / 'src/pyisam/backend'
      config.builddir = topdir / 'build/lib'
      config.libvbisam = topdir / 'lib/libvbisam'
      config.libifisam = topdir / 'lib/libifisam/64'
      config.sep_ifisam_incl = True
      config.libdisam = topdir / 'lib/libdisam'
    case 'subproject':
      topdir = config.builddir
      config.workdir = topdir / '
