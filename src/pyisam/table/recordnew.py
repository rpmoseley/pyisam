...
This version makes extensive use of annotated fields to enable the associated namedtuple
and ordering of fields with the record to be defined without having to generate source
code which is then exec'd.
...
or using make_record:

make_record(recname: str, dbname: str, prefix: str, *fields: list[tuple[str, Callable]|tuple[str, Callable, Callable]) -> tuple:
make_record(
  'DEFILErecord',               ==> recname
  'utool',                      ==> dbname
  'def',                        ==> prefix
  ('filename', TextColumn, 9),  ==> fields
  ('seq',      ShortColumn),
  ('field',    TextColumn, 9),
  ('refptr',   TextColumn, 9),
  ('type',     CharColumn),
...
make_record is a wrapper function that manipulates the field information into a format as
expected by the make_dataclass function but avoiding boilerplate coding. The example call
would become:

dataclasses.make_dataclass('DEFILErecord',
                           [('filename', TextColumn,  field(default=TextColumn(9)),
                            ('seq',      ShortColumn, field(default=ShortColumn()),
                            ('field',    TextColumn,  field(default=TextColumn(9)),
                            ('refptr',   TextColumn,  field(default=TextColumn(9)),
                            ('type',     CharColumn,  field(default=CharColumn()),
                            ('size',     ShortColumn, field(default=ShortColumn()),
                            ('keytype',  CharColumn,  field(default=CharColumn()),
                            ('vseq',     ShortColumn, field(default=ShortColumn()),
                            ('stype',    ShortColumn, field(default=ShortColumn()),
                            ('scode',    CharColumn,  field(default=CharColumn()),
                            ('fgroup',   TextColumn,  field(default=TextColumn(10)),
                            ('idxflag',  CharColumn,  field(default=CharColumn()),
                            ('_database', ],
                           (ISAMrecordBase,),
                           ISAMrecordNS)
'''
...
class _BaseColumn:
  ...
  def __get__(self, inst, objtype):
    ...
    if inst is None:
      # Return the default value for field, used by dataclass
      if hasattr(self, '_nullval'):
        return self._nullval() if callable(self._nullval) else self._nullval
      return None
...
class ISAMrecordBase:
  ...
  def __init__(self, recname, fields=None):
    ...
    if fields is None:
      fields = list(self._flddict.keys())
