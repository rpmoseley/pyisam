...
_all__ = ('decimal', 'keypart', 'keydesc' ,'dictinfo',
          'ISAMcommonMixin', 'ISAMindexMixin')

def _recbuff(recsize):
  return create_string_buffer(recsize + 1)
...
class ISAMcommonMixin:
  ...
  def issetunique(self, uniqnum):
    ...
    if self._fd is None: raise IsamNotOpen
    if self._fdmode is OpenMode.ISINPUT: raise IsamNotWritable
  ...
  def isstart(self, kdesc, mode, recbuff, keylen=0):
    ...
    self._isstart(self._fd, kdesc, keylen, recbuff, mode.value)
